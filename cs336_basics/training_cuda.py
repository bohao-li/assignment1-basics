...
from bohao.experiment.cs336_basics.transformer_lm import TransformerLanguageModel
...
def transformer_lm(
    ...
    """
    logging.info(
        f"Instantiating TransformerLanguageModel with vocab_size={vocab_size}, d_model={d_model}, "
...
class MemoryMappedDataset(torch.utils.data.Dataset):
    def __init__(self, filepath, sequence_length, dtype=np.uint16):
        ...
        logging.info(
            f"Memory-mapped dataset loaded. Total tokens: {len(self.data)}, "
...
class TrainingConfig:
    def __init__(
        ...
        special_tokens=[
            "<|endoftext|>"
        ],  # Not strictly used in this script but good to keep
        train_data_path="",
        val_data_path="",
...
def train(config: TrainingConfig):
    """
    Runs the main training loop with mixed precision.
    """
    logging.info(f"Starting training with the following configuration:\n{config}")

    # 1. Initialize GradScaler for mixed precision
    scaler = GradScaler()
    ...
    for epoch in range(config.epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            # 5. Use autocast context manager
            with torch.amp.autocast(device_type="cuda"):
                logits = model(inputs)
                loss = loss_fn(logits.view(-1, config.vocab_size), targets.view(-1))
                loss = loss / config.gradient_accumulation_steps

            # 6. Scale the loss and call backward()
            scaler.scale(loss).backward()

            if (batch_idx + 1) % config.gradient_accumulation_steps == 0:
                # 7. Unscale gradients and perform the optimizer step
    ...
    with torch.no_grad():
        for inputs, targets in data_loader:
            ...
            total_loss += loss.item() * inputs.size(
                0
            )  # Multiply by batch size for correct average
...
def save_checkpoint(model, optimizer, config: TrainingConfig, step, val_loss):
    ...
    checkpoint_name = (
        f"{config.model_name}_step_{step}_loss_{val_loss:.4f}_{timestamp}.pt"
    )
...
def main():
    ...
    config = TrainingConfig(
        ...
        d_model=2048,  # Tiny model dimension
        num_layers=10,  # Very few layers
        num_heads=8,  # Few attention heads
        d_ff=8192,  # Small feed-forward dimension
        rope_theta=10000.0,
        learning_rate=1e-4,
