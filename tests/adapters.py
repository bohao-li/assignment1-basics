...
def run_linear(
    ...
    linearModule = Linear(d_in, d_out)
    state_dict = {'W': weights}
    linearModule.load_state_dict(state_dict)
    return linearModule.forward(in_features)
...
def run_transformer_lm(
    ...
    transformer_lm_module = TransformerLanguageModel(vocab_size, d_model, num_layers, context_length, num_heads, d_ff, rope_theta)
    transformer_lm_module.load_state_dict(weights, strict=False)
    return transformer_lm_module.forward(in_indices)
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    from cs336_basics.softmax import run_softmax
    return run_softmax(in_features, dim)
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    from cs336_basics.gradient_clipping import run_gradient_clipping
    return run_gradient_clipping(parameters, max_l2_norm)
